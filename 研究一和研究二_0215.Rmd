...
将这些名字保存成rds
```{r}
saveRDS(l_变量组, "report/prepared_data/l_变量组_0215.rds")
```
...
```{r}
# 基线人口学和实验室检查比较
df_grpcop_log %>% write.csv("report/prepared_data/df_基线_随访_单因素_log.csv", row.names = F)
```

-基线数据多因素分析, logistics回归
```{r}
df_data_glm_log %>% write.csv("report/prepared_data/df_基线_多因素_log.csv", row.names = F)
```
...
-随访的实验室比较
```{r}
df_data_trp %>% write.csv("report/prepared_data/df_随访_单因素_实验室_log.csv", row.names = F)
```

-随访: 实验室比较的差异作图
```{r}
df_差异作图 %>% write.csv("report/prepared_data/df_差异作图.csv", row.names = F)
```

将上面这些表格也保存成rds
```{r}
l_数据框集合 = list(
  df_基线_随访_单因素_log = df_grpcop_log, 
  df_基线_多因素_log = df_data_glm_log, 
  f_随访_单因素_实验室_log = df_data_trp, 
  df_差异作图 = df_差异作图
)
```
...
```{r}
l_数据框集合 %>% saveRDS("report/prepared_data/l_数据框集合_0215.rds")
```
